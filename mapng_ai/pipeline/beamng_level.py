...
def write_level_package(
    *,
    ...
    # Build the terrain material set. Try the vanilla Industrial PBR pack
    # first (dramatic quality boost — full base/detail/macro layers per
    # class). Falls back to a single composite texture if vanilla isn't
    # available or MAPNG_TERRAIN_SIMPLE=1 is set.
    use_simple = os.environ.get("MAPNG_TERRAIN_SIMPLE") == "1"
    _splat_for_export = splat
    vanilla_pack = None  # (materials_dict, files_to_bundle) or None

    if splat is not None and not use_simple:
        from mapng_ai.pipeline.beamng_vanilla_terrain import (
        ...
        if vanilla_pack is not None:
            ...
            splat = None  # we'll write our own materials.json from vanilla_pack

    if vanilla_pack is None:
        # Fallback: single MapNG_Terrain covering the whole map with the
        # rendered composite texture. Self-contained, no vanilla deps.
    ...
    # ------------------------------------------------------------------
    # Terrain materials. If we successfully extracted the vanilla pack,
    # ship those (with rewritten paths + bundled textures). Otherwise
    # fall back to our single composite-texture material.
    # ------------------------------------------------------------------
    if vanilla_pack is not None:
        vanilla_mats, vanilla_files = vanilla_pack
        w(f"{base}/art/terrains/main.materials.json",
          json.dumps(vanilla_mats, indent=2))
        # Bundle every non-redirected texture from Industrial.zip
        for relpath, content in vanilla_files:
            w(relpath, content)
    else:
        w(f"{base}/art/terrains/main.materials.json",
